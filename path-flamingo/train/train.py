...
def main():
    ...
    parser.add_argument("--batch_size_tcga", type=int, default=128)
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="we define an 'epoch' as a fixed number of examples (train_num_samples_tcga, train_num_samples_gtex), not a pass through the entire dataset",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="path to TCGA H5 files with TITAN features",
    )
    parser.add_argument(
        "--gtex_vision_features",
        type=str,
        help="path to GTEX H5 files with TITAN features",
    )
    parser.add_argument(
        "--gtex_jsonl_file",
        type=str,
        help="path to GTEX JSONL, which contains file_path and results",
    )
    parser.add_argument(
        "--tcga_jsonl_file",
        type=str,
        help="path to TCGA JSONL, which contains file_path and results",
    )
    ...
    assert (args.train_num_samples_tcga // args.batch_size_tcga) == (
        args.train_num_samples_gtex // args.batch_size_gtex
    ), "number of samples per epoch must be equal for tcga and gtex"
    ...
    if args.fsdp:
        ...
        ) ## {sum(p.numel() for p in model.parameters())} --> कितने Parameters हैं Model में.
        ...
        wrapper_kwargs = dict(
            ...
            sharding_strategy=ShardingStrategy.FULL_SHARD ## do we need this ??
    ...
    tcga_feature_loader = create_feature_loader(args.tcga_vision_features)
    gtex_feature_loader = create_feature_loader(args.gtex_vision_features)

    tcga_dataset = get_data(args, tcga_feature_loader, tokenizer, "tcga")
    gtex_dataset = get_data(args, gtex_feature_loader, tokenizer, "gtex")
    total_training_steps = (
        (args.train_num_samples_tcga) // (args.batch_size_tcga * args.world_size)
    ) * args.num_epochs
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        tcga_dataset.set_epoch(epoch)
        tcga_loader = tcga_dataset.dataloader
        gtex_dataset.set_epoch(epoch)
        gtex_loader = gtex_dataset.dataloader
        ...
        train_one_epoch(
            ...
            tcga_loader=tcga_loader,
            gtex_loader=gtex_loader,
            device_id=device_id,
            wandb=wandb,
        )
